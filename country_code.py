...
#  - a hash table mapping country code to poplation
...
# with the new population. As well, clear the messaage about the 
...
def showPopulation(*args):
    ...
    if len(idxs) == 1:
        idx = int(idxs[0])
        codes = countrycodes[idx]
        name = countrynames[idx]
        popn = populations[codes]
        statusmsg.set("The population of %s (%s) is %d" % (name, codes, popn))
    sentmsg.set('')
...
#
# Figure out which country is selected, which gift is slected with the
...
# We're using the StrinVar() 'cnames', constructed from 'countrynames'
...
# fired when user makes a change, we explicitly call showPopulation
gift.set('card')
sentmsg.set('')
statusmsg.set('')
